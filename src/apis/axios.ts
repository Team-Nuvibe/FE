...
const isTokenExpiredError = (error: unknown): boolean => {
  ...
  // 400 에러 중 "만료된 jwt 토큰입니다." 또는 "유효하지 않은 jwt 토큰입니다." 메시지가 있는 경우
  if (
    status === 400 &&
    (message.includes("만료된 jwt 토큰") ||
      message.includes("유효하지 않은 jwt 토큰"))
  ) {
    return true;
  }
        ...
        refreshPromise = (async () => {
          try {
            ...
            if (!newAccessToken || !newRefreshToken) {
              throw new Error("Failed to receive new tokens");
            }
