...
const TribechatPage = () => {
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col bg-[#121212] text-white">
      {/* 헤더 영역 */}
      {/* Top: 70px explicitly requested for content start. 
          Scrap Icon: Top 73px absolute.
      */}
      {/* 헤더 영역 - 리스트의 위치를 정확하게 잡기 위해 absolute 사용 */}
      <div className="absolute top-0 left-0 z-10 flex w-full items-center justify-center bg-[#121212] px-4 pt-[70px] pb-6">
        {/* 탭 전환 (중앙 정렬, 122px width) */}
        <div className="relative h-[29px] w-[122px] cursor-pointer select-none">
          {activeTab === "ing" ? (
            <IconChatActive className="h-full w-full" />
          ) : (
            <IconChatInactive className="h-full w-full" />
          )}

          {/* 숨겨진 클릭 영역 */}
          <div className="absolute inset-0 flex">
            <div className="flex-1" onClick={() => setActiveTab("ing")} />
            <div className="flex-1" onClick={() => setActiveTab("waiting")} />
          </div>
        </div>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="pointer-events-none fixed bottom-[100px] left-1/2 z-50 flex h-[48px] w-[344px] items-center justify-center rounded-[5px] bg-[#D0D3D7]/85 py-[10px] shadow-[0_4px_4px_0_rgba(0,0,0,0.25),0_1px_3px_0_rgba(18,18,18,0.3)] backdrop-blur-[30px]"
          >
            <span className="text-center text-[14px] leading-[150%] font-[400] tracking-[-0.025em] text-[#121212]">
              {toastMessage}
            </span>
          </motion.div>
        )}
