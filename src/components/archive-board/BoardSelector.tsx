...
// Quick drop mode props (when uploading new image)
interface QuickDropMode {
  image: Blob | null;
  imageUrl: string;
  tag: string;
...
// Move mode props (when moving existing images)
interface MoveMode {
  onSelect: (selectedBoard: Board) => void;
  onClose: () => void;
}

type BoardSelectorProps = Partial<QuickDropMode> & Partial<MoveMode>;
...
export const BoardSelector = (props: BoardSelectorProps) => {
  const { imageUrl, tag, onNext, onPrevious, onSelect, onClose } = props;

  // Determine mode based on props
  const isQuickDropMode = !!onNext && !!onPrevious;
  const isMoveMode = !!onSelect && !!onClose;
  ...
  return (
    <div className="relative">
      {/* Quick Drop Mode - Show image preview and compact board list */}
      {isQuickDropMode && !showingSavedBoards && (
        <>
          <header className="flex items-center justify-between px-4 pt-2 pb-6 tracking-tight">
            ...
            <p
              ...
              onClick={() => selectedBoard && onNext?.(selectedBoard)}
            >
              완료
            </p>
          </header>
      ...
      {/* Move Mode - Show full board list directly */}
      {isMoveMode && (
        <>
          <header className="flex items-center justify-between px-4 pt-2 pb-6 tracking-tight">
            <IconXbuttonGray3
              ...
              onClick={() => onClose?.()}
            />
            <h2 className="H2 text-white">이동할 아카이브 보드</h2>
          ...
          <div className="mx-4 flex flex-col gap-3">
            {boards.map((board) => (
              <div key={board.id} className="flex flex-col gap-[10px]">
                ...
                >
                  <div
                    className={`relative h-[74px] w-[74px] shrink-0 overflow-hidden rounded-[5px] border-[0.5px] border-gray-700 bg-gray-900`}
                  >
                    {board.thumbnailUrl && (
                      <>
                        {/* 이미지 레이어 */}
                    ...
                    {!board.thumbnailUrl && (
                      <IconBoardDefault className="h-[74px] w-[74px]" />
                    )}
                  </div>
      ...
      {/* Quick Drop Mode - Full Board List View */}
      {isQuickDropMode && showingSavedBoards && (
        <>
          <header className="flex items-center justify-between px-4 pt-2 pb-6 tracking-tight">
            <IconXbuttonGray3
